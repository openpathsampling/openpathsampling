...
class ObjectAttributeStore(UniqueNamedObjectStore):
    """
    ObjectStore to store a dict with StorableObject : value
    """
    def __init__(self, substore):
        super(ObjectAttributeStore, self).__init__(
            CollectiveVariable
        )
    ...
    def store(self):
        if self._store is None:
            self._store = getattr(self.storage, self.substore)
    ...
    def _load(self, idx):
        ...
        cache_store = self.store.get_cv_cache(idx)
    ...
    def sync(self, cv):
        ...
        """
        self.store.sync_cv(cv)

    def complete(self, cv):
        self.store.complete_cv(cv)
    ...
    def cache_store(self, cv):
        ...
        """
        return self.store.cv_list[cv][0]
    ...
    def has_cache(self, cv):
        ...
        """
        return cv in self.store.cv_list
    ...
    def fill_cache(self, cv):
        ...
        """
        store = self.store.cv_list.get(cv)
        if store is not None:
            store.fill_cache()


class CVStore(ObjectAttributeStore):
    """
    ObjectStore to store a dict with StorableObject : value
    """
    def __init__(self):
