...
class Storage(NetCDFPlus):
    ...
    def _create_storages(self):
        ...
        self.create_store(
            'snapshots',
            paths.storage.FeatureSnapshotStore(self._template.__class__)
        )
    ...
    def _initialize(self):
        ...
        if template.topology is not None:
            ...
            raise RuntimeError(
                "A Storage needs a template snapshot with a topology"
            )
        ...
        # since we want to store stuff we need to finalize stores that have
    ...
    def _restore(self):
        ...
        try:
            ...
            raise RuntimeError(
                'Cannot restore storage. Some of the necessary classes '
                '(Engines, Snapshots, Topologies) require to be imported '
                'separately. So you need to run certain engine imports first. '
                'The most common way to do so is to run '
                '`import openpathsampling.dynamics.engine`'
            )
