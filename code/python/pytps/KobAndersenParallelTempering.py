...
class TransitionPathSampling(object):
    ...
    def __init__(self, system, N, NA, mass, epsilon, sigma, timestep, nsteps_per_frame, nframes, temperature, s, platform=None):
        ...
        for atom_index in range(N):
            ...
            self.sqrt_kT_over_m[atom_index,:] = units.sqrt(kT / mass) # standard deviation of velocity distribution for each coordinate for this atom
    ...
    def generateTrajectory(self, x0, nframes):
        ...
        nparticles = self.system.getNumParticles()
        mass = units.Quantity(numpy.zeros([nparticles,3], numpy.float64), units.amu)
...
def parallel_tempering_driver():
    ...
    Tmax = 2.0 * epsilon / kB
    Tmin = 0.4 * epsilon / kB
    nreplicas = 20 # number of temperatures

    # Initialize parallel tempering simulation
    print "Initializing parallel tempering simulation..."
    simulation = repex.ParallelTempering(system, coordinates, ncfilename, Tmin=Tmin, Tmax=Tmax, ntemps=nreplicas)
    simulation.number_of_iterations = 1000 # number of iterations
    simulation.number_of_equilibration_iterations = 0 # number of equilibration iterations
    simulation.timestep = timestep # timestep
    simulation.nsteps_per_iteration = nsteps_per_frame # number of steps per iteration
...
if __name__ == "__main__":
    ...
    # Run parallel tempering driver
    parallel_tempering_driver()
